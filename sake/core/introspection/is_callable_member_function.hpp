/*******************************************************************************
 * core/introspection/is_callable_member.hpp
 *
 * Copyright 2011, Jeffrey Hellrung.
 * Distributed under the Boost Software License, Version 1.0.  (See accompanying
 * file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
 *
 * #define SAKE_INTROSPECTION_DEFINE_IS_CALLABLE_MEMBER()
 *
 ...
 * This defines an interface to allow one to easily define a is_callable_xxx
 * metafunction, which determines (up to certain language limitations) whether
 * one can call a member function (usually) or member object xxx on a given
 * object type, optionally with a given signature.  One defines certain
 * predetermined macros to specify the desired member, then #include's
 * SAKE_INTROSPECTION_DEFINE_IS_CALLABLE_MEMBER().  This generates a
 ...
 *     Expands to the name of the member (as it would appear in a declaration)
 *     (e.g., "xxx").
 * SAKE_INTROSPECTION_MEMBER_DEFAULT_SIGNATURE( T )
 *     [ defaults to "void" ]
 *     Expands to the default for the Signature template parameter.
 * SAKE_INTROSPECTION_MEMBER_ARITY_LIMITS
 *     [ defaults to "( 0, SAKE_INTROSPECTION_MEMBER_DEFAULT_MAX_ARITY )" ]
 *     Expands to a Boost.Preprocessor 2-tuple giving the (inclusive) arity
 *     limits of the trait.
 * SAKE_INTROSPECTION_BUILTIN_HAS_MEMBER( T, Signature, ResultMetafunction )
 *     [ defaults to "boost::false_type" ]
 *     Expands to a type with a nested type typedef indicating whether the
 *     builtin type T has the given "member" callable with the given signature
 *     (meaningful only for operators which must be overloaded as member
 *     functions, e.g., operator[]).  The result type of calling the "member"
...
#ifndef SAKE_CORE_INTROSPECTION_IS_CALLABLE_MEMBER_HPP
#define SAKE_CORE_INTROSPECTION_IS_CALLABLE_MEMBER_HPP
...
#ifndef SAKE_INTROSPECTION_MEMBER_DEFAULT_MAX_ARITY
#define SAKE_INTROSPECTION_MEMBER_DEFAULT_MAX_ARITY 8
#endif // #ifndef SAKE_INTROSPECTION_MEMBER_DEFAULT_MAX_ARITY

#define SAKE_INTROSPECTION_DEFINE_IS_CALLABLE_MEMBER() \
    <sake/core/introspection/private/is_callable_member.ipp>

#endif // #ifndef SAKE_CORE_INTROSPECTION_IS_CALLABLE_MEMBER_HPP
