...
public:
    ...
    SAKE_MEMBERWISE_SWAP( typename adaptor, (( member_base_ )) )
...
#include SAKE_OVERLOAD_GENERATE()
    ...
    // RandomAccess
    template< class T >
    void derived_advance_ip(T const n)
    {
        BOOST_STATIC_ASSERT((boost::is_same< T, difference_type >::value));
        member_base_::protected_base() += n;
    }

    template< class Other >
    bool derived_less(Other const & other) const
    { return base() < adaptor_private::as_convertible_relax<I>(other); }
    template< class Other >
    ...
    sake::sign_t derived_cmp(Other const & other) const
    {
        return sake::cmp(
            base(),
            adaptor_private::as_convertible_relax<I>(other)
        );
    }
