...
struct primary_rv_sink
{
    ...
    // GCC <= 4.6.3 requires primary_rv_sink to be copy constructible in order
    // for function arguments to bind to it.
    SAKE_NONCOPY_ASSIGNABLE( primary_rv_sink )
#else // #if SAKE_GNUC_VERSION && SAKE_GNUC_VERSION <= SAKE_GNUC_VERSION_OF(4,6,3)
